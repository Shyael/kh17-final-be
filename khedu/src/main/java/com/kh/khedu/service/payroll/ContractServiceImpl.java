...
public class ContractServiceImpl implements ContractService {
	...
	public void update(
		...
		if(currentContract.getContractEnd() != null
				&& currentContract.getContractEnd()
	...
	public void employeeSign(
		...
		// [3] 직원 서명 설정
		currentContract.setEmployeeSignature(
				request.getEmployeeSignature()
		);
		...
		// 암호화는 DAO에서 처리
		boolean result =
				contractDao.employeeSign(currentContract);
		...
		if(currentContract.getEmployerSignature() != null) {
			...
			boolean completeResult =
					contractDao.completeSign(currentContract);
	...
	public void employerSign(
		...
		// [3] 원장 서명 설정
		currentContract.setEmployerSignature(
				request.getEmployerSignature()
		);
		...
		// 암호화는 DAO에서 처리
		boolean result =
				contractDao.employerSign(currentContract);
	...
	public ContractDto findCurrent(long employeeNo) {
		...
		ContractDto contractDto =
				contractDao.findCurrent(employeeNo);
	...
	public List<ContractDto> findPast(long employeeNo) {
		...
		List<ContractDto> history =
				contractDao.findPast(employeeNo);
	...
	public void endContract(long contractNo) {
		...
		// [1] 종료할 근로계약 조회
		ContractDto contractDto =
				contractDao.findPeriodAndStatus(contractNo);
		...
		// [4] 근로계약 종료 처리
		boolean result =
				contractDao.changeStatus(contractDto);
	...
	public ContractDto changeWorkCondition(
		...
		boolean changeResult =
				contractDao.changeStatus(originDto);
	...
	public ContractSignResponseVO findSignature(
		...
		return ContractSignResponseVO.builder()
